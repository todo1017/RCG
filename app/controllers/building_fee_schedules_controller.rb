class BuildingFeeSchedulesController < ApplicationController
  ...
  def index
    @building_fee_schedules = BuildingFeeSchedule.joins(:building).where("building_id IN (#{@current_user_buildings})").order("buildings.geography_id, buildings.competitor asc")
